...
class BuyCommand extends Command
{
	...
	public function execute(CommandSender $sender, string $label, array $args): bool
	{
		...
		$sender->sendMessage(SMarket::$prefix . "구매 전 : " . $this->owner->getEconomyAPI()->getPlayerPreferredCurrency($sender, false)->format($money_before) . "  /  구매 후 : §a" . $this->owner->getEconomyAPI()->getPlayerPreferredCurrency($sender)->format($money_after) . "§7  /  소비한 금액 : §a" . $this->owner->getEconomyAPI()->getPlayerPreferredCurrency($sender)->format($money_before - $money_after) . "§7");
