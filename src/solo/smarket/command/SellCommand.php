...
class SellCommand extends Command{
	...
	public function execute(CommandSender $sender, string $label, array $args) : bool{
		...
		if($market === null){
			...
			if($itemInHand->getId() !== ItemIds::AIR){
		...
		$sender->sendMessage(SMarket::$prefix . "판매 전 : " . $this->owner->getEconomyAPI()->getPlayerPreferredCurrency($sender, false)->format($money_before) . "  /  판매 후 : §a" . $this->owner->getEconomyAPI()->getPlayerPreferredCurrency($sender)->format($money_after) . "§7  /  얻은 금액 : §a" . $this->owner->getEconomyAPI()->getPlayerPreferredCurrency($sender)->format($money_after - $money_before) . "§7");
