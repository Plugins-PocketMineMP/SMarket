	...
	public function onRun(): void
	{
		...
		while ($this->inventoryIndex < $this->player->getInventory()->getSize()) {
			...
			if ($this->owner->getSetting()->get("sell-all-at-once")) {
				...
				$this->player->sendPopup("§b" . $itemname . " " . $content->getCount() . "개§7를 " . $this->owner->getEconomyAPI()->getPlayerPreferredCurrency($this->player, false)->format($earned) . "에 판매하였습니다.(합: " . $this->owner->getEconomyAPI()->getPlayerPreferredCurrency($this->player)->format($this->totalEarned) . ")");
				return; // continue sell(Delayed)
			}
		}
		// end
		$this->player->sendMessage(SMarket::$prefix . "판매된 아이템 : " . implode(", ", array_map(function ($key, $value) {
				return $key . " " . $value . "개";
			}, array_keys($this->totalSelled), $this->totalSelled)));
		$this->player->sendMessage(SMarket::$prefix . "판매 합계 : " . $this->owner->getEconomyAPI()->getPlayerPreferredCurrency($this->player, false)->format($this->totalEarned));
		$this->close();
	}
